...
class Sarsa:
    ...
    def update_model(self, state, action, reward, next_state, next_action):
        """
        ...
        next_statereward = self._qmatrix.ix[next_action, next_state]
        r = reward
        alpha = self._learn_rate
        gamma = self._discount_factor

        # Do the computation
        new_q_sa = q_sa + alpha * (r + gamma * next_statereward - q_sa)
        
        self._qmatrix.iat[action, state] = new_q_sa
    def epsilon_greedy_action(self,state):
        return self.get_best_action(state) if (np.random.rand()) < (1-self.epsilon) else np.random.randint(0,3+1)
